...
public class DynamicQueryTest {
	...
	public void testIsGenerateProperTwoCoditionSQL() {
		...
		argumentsList.add(new SQLGeneratorParameter("chajei", "like", cangjie)
				.wrapValueWithPercentSign(SQLGeneratorParameter.PercentSignEnum.前後));
		argumentsList.add(new SQLGeneratorParameter("phon", "like", phonetic)
				.wrapValueWithPercentSign(SQLGeneratorParameter.PercentSignEnum.前後));
		argumentsList.add(new SQLGeneratorParameter("cns", "like", cns_code)
				.wrapValueWithPercentSign(SQLGeneratorParameter.PercentSignEnum.前後));
	...
	public void testIsGenerateProperThreeCoditionSQL_prefixQueryConditionNeedToBeEmptyString() {
		...
		// 假設prefix這個欄位 就算使用者傳入空字串 也會成為合理的查詢條件 就在後面再加上
		// .valueToBeAssignedNeedToBeEmptyString()
	...
	public void testIsGenerateCorrectRowNumberSQL() {
		...
		// 假設這個分頁查詢handler裡，前端有可能會傳來這些參數，但是有些可能有輸入，有些可能沒有，因為這是個複合查詢(composite
		// query)的使用情境
		int startRow = 1;
		int endRow = 5;
		String county = "E";
		...
		/*
		 * select * from ( select *, row_number() over(order by big5) as rn from
		 * EUDCFONT %s) as A where A.rn between ? and ? %s 會被換成 where cty = ? chars = ?
		 * ..... 根據使用者先前給的指示，有合理的值的查詢條件才會被加入where子句中，過程中完全不用自己去寫 if(!"".equals(cty) ||
		 * cty != null){...} 這樣的程式碼
		 * 
		 * 再使用 Apache DbUtils 或 Spring JDBCTemplate之類的library進行DB操作 QueryRunner run =
		 * new QueryRunner( aDataSource ); run.query(tempSQL, rs -> {.....},
		 * okParameterValues.toArray());
		 */
		assertEquals(String.format(GET_EUDCFONT_BY_ROWNUMBER_BASESQL, theWhereClause), tempSQL);
		assertArrayEquals(okParameterValues.toArray(), new Object[] { county, stroke, prefix,
				String.format("%%%s%%", cangjie), String.format("%%%s%%", phonetic), startRow, endRow });
	...
	public void queryAllRareWordsData() {
		...
		assertEquals(String.format("%s where cty = ? and chars = ? and prefix = ? and chajei like ? and cns like ?",
				GET_All_EUDCFONT_BASESQL), sql);
		assertArrayEquals(okParameterValues.toArray(),
				new Object[] { county, stroke, prefix, String.format("%%%s%%", cangjie), String.format("%%%s%%", cns_code) });
